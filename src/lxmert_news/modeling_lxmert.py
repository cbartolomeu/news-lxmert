...
from .configuration_lxmert import LxmertNewsConfig
...
@dataclass
class LxmertNewsModelOutput(ModelOutput):
    """
...
@dataclass
class LxmertNewsForQuestionAnsweringOutput(ModelOutput):
    """
...
@dataclass
class LxmertNewsForPreTrainingOutput(ModelOutput):
    """
...
class LxmertNewsEmbeddings(nn.Module):
...
class LxmertNewsAttention(nn.Module):
...
class LxmertNewsAttentionOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class LxmertNewsCrossAttentionLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.att = LxmertNewsAttention(config)
        self.output = LxmertNewsAttentionOutput(config)
...
class LxmertNewsSelfAttentionLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self = LxmertNewsAttention(config)
        self.output = LxmertNewsAttentionOutput(config)
...
class LxmertNewsIntermediate(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class LxmertNewsOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class LxmertNewsLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.attention = LxmertNewsSelfAttentionLayer(config)
        self.intermediate = LxmertNewsIntermediate(config)
        self.output = LxmertNewsOutput(config)
...
class LxmertNewsXLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        # The cross-attention Layer
        self.visual_attention = LxmertNewsCrossAttentionLayer(config)

        # Self-attention Layers
        self.lang_self_att = LxmertNewsSelfAttentionLayer(config)
        self.visn_self_att = LxmertNewsSelfAttentionLayer(config)

        # Intermediate and Output Layers (FFNs)
        self.lang_inter = LxmertNewsIntermediate(config)
        self.lang_output = LxmertNewsOutput(config)
        self.visn_inter = LxmertNewsIntermediate(config)
        self.visn_output = LxmertNewsOutput(config)
...
class LxmertNewsVisualFeatureEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        feat_dim = config.visual_feat_dim
        pos_dim = config.visual_pos_dim
        faces_feats_dim = config.visual_feat_faces_dim
        ...
        self.dropout_regions = nn.Dropout(config.hidden_dropout_prob)

        # Faces feature encoding
        self.visn_faces_fc = nn.Linear(faces_feats_dim, config.hidden_size)
        self.visn_faces_layer_norm = nn.LayerNorm(config.hidden_size, eps=1e-12)

        self.dropout_faces = nn.Dropout(config.hidden_dropout_prob)

    def forward(self, visual_feats, visual_pos, faces_feats):
        ...
        output = self.dropout_regions(output)

        z = self.visn_faces_fc(faces_feats)
        z = self.visn_faces_layer_norm(z)

        output_faces = self.dropout_faces(z)

        return torch.cat((output, output_faces), 1)


class LxmertNewsEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()

        # Obj-level image embedding layer
        self.visn_fc = LxmertNewsVisualFeatureEncoder(config)
        self.config = config
        ...
        self.layer = nn.ModuleList([LxmertNewsLayer(config) for _ in range(self.num_l_layers)])
        self.x_layers = nn.ModuleList([LxmertNewsXLayer(config) for _ in range(self.num_x_layers)])
        self.r_layers = nn.ModuleList([LxmertNewsLayer(config) for _ in range(self.num_r_layers)])
    ...
    def forward(
        ...
        visual_feats = self.visn_fc(visual_feats, visual_pos, faces_feats)
        # Combine attention masks to match visual feats shape
        # attentions have shape (batch_size, 1, 1, seq_len)
        visual_attention_mask = visual_attention_mask if faces_attention_mask is None \
            else torch.cat((visual_attention_mask, faces_attention_mask), 3)
...
class LxmertNewsPooler(nn.Module):
    def __init__(self, config):
        super(LxmertNewsPooler, self).__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
...
class LxmertNewsPredictionHeadTransform(nn.Module):
    def __init__(self, config):
        super(LxmertNewsPredictionHeadTransform, self).__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
...
class LxmertNewsLMPredictionHead(nn.Module):
    def __init__(self, config, lxmert_model_embedding_weights):
        super(LxmertNewsLMPredictionHead, self).__init__()
        self.transform = LxmertNewsPredictionHeadTransform(config)
...
class LxmertNewsVisualAnswerHead(nn.Module):
...
class LxmertNewsFacesHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.transform = LxmertNewsPredictionHeadTransform(config)
        # Decide the use of visual losses
        faces_losses = {}
        if config.faces_loss:
            faces_losses["faces_feat"] = {
                "shape": (-1, config.visual_feat_faces_dim),
                "num": config.visual_feat_faces_dim,
            }
        self.faces_losses = faces_losses

        # The output weights are the same as the input embeddings, but there is
        # an output-only bias for each token.
        self.decoder_dict = nn.ModuleDict(
            {key: nn.Linear(config.hidden_size, self.faces_losses[key]["num"]) for key in self.faces_losses}
        )

    def forward(self, hidden_states):
        hidden_states = self.transform(hidden_states)
        output = {}
        for key in self.faces_losses:
            output[key] = self.decoder_dict[key](hidden_states)
        return output


class LxmertNewsVisualObjHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.transform = LxmertNewsPredictionHeadTransform(config)
        # Decide the use of visual losses
...
class LxmertNewsPreTrainingHeads(nn.Module):
    def __init__(self, config, lxmert_model_embedding_weights):
        super(LxmertNewsPreTrainingHeads, self).__init__()
        self.predictions = LxmertNewsLMPredictionHead(config, lxmert_model_embedding_weights)
...
class LxmertNewsPreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = LxmertNewsConfig
...
class LxmertNewsModel(LxmertNewsPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.embeddings = LxmertNewsEmbeddings(config)
        self.encoder = LxmertNewsEncoder(config)
        self.pooler = LxmertNewsPooler(config)
        self.init_weights()
    ...
    @add_code_sample_docstrings(
        ...
        output_type=LxmertNewsModelOutput,
    ...
    def forward(
        ...
        # Process the faces attention mask
        if faces_attention_mask is not None:
            extended_faces_attention_mask = faces_attention_mask.unsqueeze(1).unsqueeze(2)
            extended_faces_attention_mask = extended_faces_attention_mask.to(dtype=self.dtype)
            extended_faces_attention_mask = (1.0 - extended_faces_attention_mask) * -10000.0
        else:
            extended_faces_attention_mask = None
        ...
        encoder_outputs = self.encoder(
            ...
            faces_attention_mask=extended_faces_attention_mask,
            output_attentions=output_attentions,
        )
...
class LxmertNewsForPreTraining(LxmertNewsPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        # Configuration
        self.config = config
        ...
        # Lxmert backbone
        self.lxmert = LxmertNewsModel(config)

        # Pre-training heads
        self.cls = LxmertNewsPreTrainingHeads(config, self.lxmert.embeddings.word_embeddings.weight)
        if self.task_obj_predict:
            self.obj_predict_head = LxmertNewsVisualObjHead(config)
            self.faces_predict_head = LxmertNewsFacesHead(config)
        if self.task_qa:
            self.answer_head = LxmertNewsVisualAnswerHead(config, self.num_qa_labels)
    ...
    @add_start_docstrings_to_model_forward(LXMERT_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
    @replace_return_docstrings(output_type=LxmertNewsForPreTrainingOutput, config_class=_CONFIG_FOR_DOC)
    def forward(
            self,
            input_ids=None,
            visual_feats=None,
        ...
        visual_output_feats = visual_output[:, 0:36, :]
        visual_output_faces = visual_output[:, 36:, :]
        ...
        if obj_labels is not None and self.task_obj_predict:
            ...
            for key, key_info in self.visual_losses.items():
                ...
                losses[key] = visual_loss
            total_loss += total_visual_loss

            # Faces Loss
            total_faces_loss = torch.tensor(0.0, device=input_ids.device)
            faces_prediction_scores_dict = self.faces_predict_head(visual_output_faces)
            for key, key_info in self.faces_losses.items():
                label, mask_conf = obj_labels[key]
                output_dim = key_info["num"]
                loss_fct_name = key_info["loss"]
                label_shape = key_info["shape"]
                weight = self.visual_loss_normalizer
                face_loss_fct = self.loss_fcts[loss_fct_name]
                visual_prediction_scores = faces_prediction_scores_dict[key]
                face_loss = face_loss_fct(
                    visual_prediction_scores.view(-1, output_dim),
                    label.view(*label_shape),
                )
                if face_loss.dim() > 1:  # Regression Losses
                    face_loss = face_loss.mean(1)
                face_loss = (face_loss * mask_conf.view(-1)).mean() * weight
                total_faces_loss += face_loss
                losses[key] = face_loss
            total_loss += total_faces_loss
        if ans is not None and self.task_qa:
            answer_loss = self.loss_fcts["ce"](answer_score.view(-1, self.num_qa_labels), ans.view(-1))
            total_loss += answer_loss
            losses["qa"] = answer_loss.detach()
        ...
        return LxmertNewsForPreTrainingOutput(
            loss=total_loss,
            losses=losses,
...
class LxmertNewsForQuestionAnswering(LxmertNewsPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        # Configuration
        self.config = config
    ...
    @add_code_sample_docstrings(
        tokenizer_class=_TOKENIZER_FOR_DOC,
        checkpoint=_CHECKPOINT_FOR_DOC,
        output_type=LxmertNewsForQuestionAnsweringOutput,
        config_class=_CONFIG_FOR_DOC,
    )
    def forward(
            self,
        ...
        return LxmertNewsForQuestionAnsweringOutput(
            loss=loss,
            question_answering_score=answer_score,
            language_hidden_states=lxmert_output.language_hidden_states,
            vision_hidden_states=lxmert_output.vision_hidden_states,
